...
function checkUserBlock(ipAddress) {
    ...
    xhr.open('GET', 'cargo-plugins/blockedUser.php', true);
    xhr.send();
...
function sendUserDataToServer() {
    ...
    xhr.open('GET', 'cargo-plugins/get-location.php', true);
    xhr.send();
        ...
        if (regexname.test(name)) {
            if (regex.test(num)) {
                if (!stopClick) {
                    ...
                    // Configure it: POST-request for the server.php
                    xhr.open("POST", "cargo-plugins/customer-whatsapp.php", true);
                    xhr.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
...
function bizWhatsapp(inputs, num, name) {
    ...
    // Configure it: POST-request for the server.php
    xhr.open("POST", "cargo-plugins/biz15-whatsapp.php", true);
    xhr.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
...
function bizMailTo(message, num) {
    ...
    // Configure it: POST-request for the server.php
    xhr.open("POST", "cargo-plugins/mailto.php", true);
    xhr.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
...
function addToContacts(name, num) {
    ...
    // Configure it: POST-request for the server.php
    xhr.open("POST", "cargo-plugins/add-to-customer-group.php", true);
    xhr.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
...
function sentSMS(name, num, inputs) {
    ...
    // Configure it: POST-request for the server.php
    xhr.open("POST", "cargo-plugins/company-sms.php", true);
    xhr.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
